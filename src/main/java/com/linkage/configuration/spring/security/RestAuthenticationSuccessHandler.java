package com.linkage.configuration.spring.security;

import com.linkage.base.SystemCode;
import com.linkage.controller.admin.course.CourseController;
...
public class RestAuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException, ServletException {
        ...
        if (null != object) {
            ...
            com.linkage.domain.User user = userService.getUserByUserName(springUser.getUsername());
            
            if (null != user) {
                UserEventLog userEventLog = new UserEventLog(user.getId(), user.getUserName(), user.getRealName(), new Date());
                logger.info("+++++++++++++++++++++++" + user.getUserName() + " 登录了凌志线上教育培训系统 at:" + System.currentTimeMillis() + "+++++++++++++++++++++++");
                userEventLog.setContent(user.getUserName() + " 登录了凌志线上教育培训系统");
                eventPublisher.publishEvent(new UserEvent(userEventLog));
                com.linkage.domain.User newUser = new com.linkage.domain.User();
